...
6. **Honest states.** Prefer named states over false greens:
   - **proven** — a named proof (AC) or `@covers`/proof (US/FR/NFR) exists (not “tests ran and passed” as a ceremony claim)
...
10. **Viewer invariant.** Gate PASS ⇔ contiguous braid in Loupe; Gate FAIL ⇔ fray / Golden Thread broken. Excused incompleteness carries its own color without fray — amber for tracked debt, blue for not-yet backlog; red is reserved for fray / refusal only.
