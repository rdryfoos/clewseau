# trace-manifest.json — SpecAssay trace-manifest schema

Portable, vendor-neutral **trace-manifest** (matrix artifact). Gate 2 always emits this file (default path `trace-manifest.json`).
...
**Invariant for viewers:** Gate PASS (`gate.ok`) ⇔ contiguous descent braid; Gate FAIL ⇔ fray / Golden Thread broken. Tracked debt and excused incompleteness may still show red nodes without fray.
