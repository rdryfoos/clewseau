...
[Draft PR #8 on HomesFlow](https://github.com/rdryfoos/HomesFlow/pull/8) — the branch as the agent left it, unedited. The agent:
...
A break/fix probe was then run against the delivered slice: renaming the proof flipped the row from `proven` to `tracked-debt` via an open `Carries:` task; restoring the name flipped it back. The Gate refused nothing falsely in either direction. The full break/fix writeup lives on the original trial PR branch.
